...
if [ $? -eq 0 ]; then
  ...
  while :; do
    ...
    for ((time = 5; time >= 0; time--)); do
      echo -ne "\r在准备正行删档操作！！，剩余 ${CBLUE}$time${CEND} 秒，可以在计时结束前，按 CTRL+C 退出！如果需要备份，退出后再执行【backup】命令\r"
      sleep 1
    done
    echo -ne "\n\r"
    echo -ne "${CYELLOW}正在删档，数据全部清空…………\r${CEND}"
    #重构前，先备份数据库以及版本数据。
    docker exec -it gsmysql /bin/sh /usr/local/bin/gsmysqlRestore.sh reset
    if [ $? -eq 0 ]; then
