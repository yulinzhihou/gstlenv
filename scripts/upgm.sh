...
if [ $? -eq 0 ]; then
  ...
  function get_distribution_package() {
    if [ ! -r /root/GSOnlineGM.tar.gz ] || [ ! -r /root/GSOnlineGM.sha256 ]; then
    ...
    # 验证完成 。创建包
    tar zxf /root/GSOnlineGM.tar.gz -C /tlgame
    chmod -R 777 /tlgame/GSOnlineGM
    chown -R root.root /tlgame/GSOnlineGM
  ...
  function set_lua_config() {
    ...
    # 查找有没有脚本ID存在
    grep -q "111111" $SCRIPT_DAT
    if [ $? -eq 0 ]; then
      # 表示存在
      grep -q "111111=\Core.lua" $SCRIPT_DAT
      if [ $? -eq 1 ]; then
        grep -q "Core.lua" $SCRIPT_DAT
        if [ $? -ne 0 ]; then
          ...
          for INDEX in "${!SCRIPT_ID[@]}"; do
            grep -q ${SCRIPT_ID[$INDEX]} $SCRIPT_DAT
            if [ $? -eq 0 ]; then
              # 替换模板文件里面的111111为指定的
  ...
  function main() {
    ...
    if [ -r $ENV_EXAMPLE_FILE ]; then
      ...
      sed -i "s/REDIS_PORT=.*/REDIS_PORT=${REDIS_HOST_PORT}/g" $ENV_FILE   # REDIS密码
      sed -i "s/DB_PASS=.*/DB_PASS='${TL_MYSQL_PASSWORD}'/g" $ENV_FILE     # web库密码
      sed -i "s/DB_PORT=.*/DB_PORT=${TL_MYSQL_PORT}/g" $ENV_FILE           # web库端口
      sed -i "s/TLBB_PASS=.*/TLBB_PASS='${TL_MYSQL_PASSWORD}'/g" $ENV_FILE # tlbbdb库密码
      sed -i "s/TLBB_PORT=.*/TLBB_PORT=${TL_MYSQL_PORT}/g" $ENV_FILE       # tlbbdb库端口
    ...
    if [ $? -eq 0 ]; then
      ...
      if [ ! -r /tlgame/tlbb/Public/Data/Script/Core.lua ]; then
        \cp -rf /tlgame/GSOnlineGM/Core.lua /tlgame/tlbb/Public/Data/Script/
      fi
