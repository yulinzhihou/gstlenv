...
- ~~第二步：完成第一步的安装结果后，会弹出提示，`gstl`  使用此命令进行游戏服务端的安装与常用配置参数的配置~~  **已经整合到第一步**

```bash
gstl
```

- ~~第三步：如果小服务器配置，如`1G` `2G` 内存的，需要拓展一下虚拟内存，才能流畅跑起服务端 **(注：此步骤超过4G内存的服务器可以不用管，必须保证服务器硬盘总容量有20G以上才能使用。不是空余空间，是总共有20G以上的就可以使用) 强烈建议不管多大内存，都需要开启一下，毕竟不要钱，只占一点空间而已**。~~   **已经整合到第一步**

```bash
swap
```

- 第四步：上传服务端到 `/root` 下，点击 `xshell` 软件的 `sftp` 按钮，进行 `sftp` 命令行传输模式 **（注：用 `winscp` 软件或者其他上传软件都行，只要把服务端上传到 `/root` 目录下即可，服务端的名称只能是  `tlbb.tar.gz` 或者 `tlbb.zip` 否则后面的操作会报错）** **如果不是用命令行上传，请不要执行此步骤命令**
