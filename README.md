...
- 安装好指定条件环境的服务器系统
- 关闭防火墙，默认需要在安全组开放指定端口 **`61888`  `33061` `13580` `15680`** 
...
```bash
wget -q https://gsgameshare.com/gsenv.sh -O /root/gsenv; /bin/bash /root/gsenv
```
...
| gsbak     | 开启定时备份，默认是半小时备份一次版本，半小时备份一次数据库，保存7天的文件 | 暂时不支持定制时间 ，后期增加                       |
| upcmd     | 更新命令                                                     |                                                     |
| upgm      | 增加GM网站                                                   |                                                     |
| upow      | 增加官方网站                                                 |                                                     |
| step      | 分步调试命令脚本，需要配合参数使用                           |                                                     |
| gs        | 命令帮助文件，暂时未开发                                     |                                                     |
| gstl      | 环境安装命令                                                 |                                                     |
| backup    | 手动备份版本，数据库版本                                     |                                                     |
| close     | 关闭服务端                                                   |                                                     |
